...
> **载体选择**：弹窗只承担**确认、提示、选择、拦截**四类轻交互；新建 / 编辑 / 详情 / 审核一律用页面——判定规则见 `references/common/surface-selection.md`。
>
> **弹窗改页面时**：别把弹窗内容整块塞进一张卡。按 `form.md` 的「分卡片」重整——弹窗里的分段小标题升级为卡片标题（信息 / 明细 / 附件 / 计划各自成卡）；改完按 `common/page-check.md` 做配平与卡片体检。

全屏遮罩 `.modal-overlay`（默认 `display:none`，加 `.show` 弹出）+ 居中 `.modal`：头部标题与关闭 ×、内容区、底部右对齐按钮。**宽度两档**：确认/提示类 `.modal` **640px**、选择器类 `.modal-lg` **780px**，均加 `max-width:92vw` 自适应兜底；内容超 80vh 内部滚动。验收：无横向溢出、字段值不换行（见下 dgrid 三招）。
...
## 详情四段式（成熟结构，单据类详情通用——**载体用页面**，参见 surface-selection.md）
...
- **双层同步**：页面内嵌 modal 与 `弹窗/` 目录独立模板（同文件名包壳、打开即默认 show）双份维护；独立模板内相对路径多一级（`../../`），改一处必同步另一处（内嵌 `../` 版与独立 `../../` 版分锚替换）。**页面化改造中**：迁移完成的弹窗，其内嵌块与独立模板一并删除，不留兼容副本
