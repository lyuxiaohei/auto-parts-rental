...
**Arrow connectivity:**
- [ ] Every arrow starts/ends at a node edge or diamond vertex — never from node center or interior?
- [ ] Horizontal arrows: x1 = source right edge (`x + width`), x2 = target left edge (`x`)?
- [ ] Vertical arrows: y1 = source bottom edge (`y + height`), y2 = target top edge (`y`)?
- [ ] Row-wrap arrows: vertical segment starts from source center bottom (`cx, y + height`), ends at target center top (`cx, y`)?
- [ ] Diamond exits originate at vertex points (left/right/bottom), not interior coordinates?
- [ ] Loop-back arrows end at the correct target node edge — not a different node or empty space?
- [ ] No orphaned arrows pointing to coordinates with no node?
