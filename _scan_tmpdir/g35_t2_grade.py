...
A = [
 ...
 ('库存查询', [('ADD', '物料类型（select·值域=stockFlows fields.cls 去重·租赁器具/零部件·与行数据同源）', 'P1', '判据2：表格有「物料类型」列（th 第3列），筛选区无类型筛选；注：产品档案 WL 字典 6 值与库存行两值口径不同——值域与行数据同源优先（取 WL 会滤空）')], ''),
 ...
 ('租入入库列表', [('ADD', '制单人（select·值域=rentInbounds fields.maker 去重）', 'P1', '判据5：表格有「制单人」列（李国栋/陈金），筛选区无人员筛选'),
                  ('ADD', '入库库区（select·值域=rentInbounds fields.area 去重）', 'P1', '判据7：表格有「入库库区」列，筛选区无库房筛选（label 对齐 th 用「入库库区」）')], ''),
 ...
 ('采购入库列表', [('ADD', '制单人（select·值域=实体 fields.maker 去重）＋DATA 补 fields.maker×8 行', 'P1', '判据5：表格有「制单人」列，筛选区无人员筛选；行数据缺 fields.maker 需补（值取自本行 cells·D-129 去业务类型后 cells 索引移位·按新序取）'),
                 ('ADD', '入库库区（select·值域=实体 fields.area 去重）＋DATA 补 fields.area×8 行', 'P1', '判据7：表格有「入库库区」列，筛选区无库房筛选；行数据缺 fields.area 需补（cells 新索引 4）')], ''),
