# -*- coding: utf-8 -*-
"""G53 acceptance data checks (read-only). Covers:
  (3) old-label zero-hit + new-label >=1 hit within the 20 G53 entities
  (5) items row length === itemCols length (3 sampled records + full sweep)
Only writes inside _scan_tmpdir/g53_accept/.
"""
import subprocess
import sys
...
DATA = ROOT / "P3-R01-包装租赁管理后台原型" / "_data" / "demo-data.js"
...
def main() -> int:
    js_path = HERE / "check_data.node.js"
    js_path.write_text(NODE_SCRIPT, encoding="utf-8", newline="\n")
    proc = subprocess.run(
        ["node", str(js_path), str(DATA)],
        capture_output=True, text=True, encoding="utf-8", errors="replace",
        cwd=str(HERE),
    )
    if proc.returncode != 0:
        print("NODE FAILED rc=%d" % proc.returncode)
        print(proc.stdout)
        print(proc.stderr)
        return 1
    (HERE / "check_data.out.json").write_text(proc.stdout, encoding="utf-8")
    print(proc.stdout)
    return 0
