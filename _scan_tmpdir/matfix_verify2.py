...
with sync_playwright() as p:
    ...
    # 1) 从库存查询页直接执行侧边栏菜单的 go()（等价点击，绕过折叠不可见）
    pg.goto((base / "仓储作业" / "库存查询.html").as_uri())
    pg.wait_for_timeout(400)
    onclick = pg.locator(".sm-link", has_text="物料档案").first.get_attribute("onclick")
    pg.evaluate(onclick)
    pg.wait_for_timeout(500)
    print(f"[1 菜单跳转] onclick={onclick} | 落地 URL 含 产品档案.html: {'产品档案.html' in str(pg.url)} | 页面 h1: {pg.title() + ' / 行数:' + str(pg.locator('tbody tr').count())}")
    # 2) 落地页 详情 弹窗
