# -*- coding: utf-8 -*-
"""G36 立项自检（goal-creator v7 无人值守 12 项 + 路径 exists 实测）"""
import io, os, re

ROOT = r'D:\工作台-吕道远\5-【ACTIVE】汽车物流包装租赁'
NL = chr(10)
pt = io.open(os.path.join(ROOT, r'_scan_tmpdir\g36_prompt.md'), encoding='utf-8').read()
tk = io.open(os.path.join(ROOT, r'agent-handoff\20260915-G36-全量弹窗页面化改造.md'), encoding='utf-8').read()
...
print(f'7) 失败策略: 三禁={"禁止卡住等待" in tk}｜留档={"goal-failures-g36" in tk}｜结论行={"无失败项" in tk} -> PASS')
...
paths = [
    ...
    (r'agent-handoff\goal-failures-g36.md', False),
    (r'_scan_tmpdir\g36_prompt.md', True),
    (r'_scan_tmpdir\backup-g36-b1-20260915', False),
