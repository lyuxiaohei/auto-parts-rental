...
'''
tmp = r'D:\工作台-吕道远\5-【ACTIVE】汽车物流包装租赁\_scan_tmpdir\g35_probe4.js'
io.open(tmp, 'w', encoding='utf-8', newline='\n').write(node_code)
r = subprocess.run(['node', tmp, ROOT + r'\_data\demo-data.js'], capture_output=True, text=True, shell=False)
print(r.stdout)
if r.returncode != 0:
    print('STDERR:', r.stderr[:800])
# 用户权限 状态 select options
t = io.open(ROOT + r'\系统管理\用户权限.html', encoding='utf-8').read()
i = t.find('>状态：')
print('用户权限 状态 select:', t[i:i+220].replace('\r\n', '\n'))
