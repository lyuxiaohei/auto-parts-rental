# -*- coding: utf-8 -*-
"""菜单重组 v3.2 · 全站侧边栏重写（40 页，含角色管理页）
相对 v3.1 差异（G02 文档 A 表）：
· 损益移出项目管理组 → 财务管理组尾普通菜单项，菜单名「项目损益」（→财务协同/盈亏报表.html）
· 组名 财务协同→财务管理、仓储作业→仓储管理（仅侧边栏标签层，目录/文件零移动）
· PAGE_STATE：财务协同六页 open=财务管理、盈亏报表 selected=项目损益+open=财务管理、仓储六页 open=仓储管理
规范块整体替换 <aside class="sidebar">...</aside>；小标签样式=旧仓储 li 原文（恰 8 个）；二进制读写保行尾。
v3.5（G10·2026-09-10）：项目损益 移至财务管理组首+改名「财务看板」（应收小标签之前·href 不动）；PAGE_STATE 清 G07 已删两台账残留行（40→38 页）。
"""
...
PAGE_STATE = {
    ...
    '仓储作业/库存查询.html': ('库存查询', '仓储管理'),
    '仓储作业/盘点列表.html': ('盘点', '仓储管理'),
    '仓储作业/盘点录入.html': ('盘点', '仓储管理'),
    '仓储作业/库存调拨列表.html': ('库存调拨', '仓储管理'),
    '仓储作业/其他入库列表.html': ('其他入库', '仓储管理'),
    '仓储作业/其他出库列表.html': ('其他出库', '仓储管理'),
    '财务协同/应收账单.html': ('应收账单', '财务管理'),
    '财务协同/开票登记.html': ('开票登记', '财务管理'),
    '财务协同/回款登记.html': ('收款登记', '财务管理'),
    '财务协同/银行水单核销.html': ('收款核销', '财务管理'),
    '财务协同/应付账单.html': ('应付账单', '财务管理'),
    '财务协同/付款登记.html': ('付款登记', '财务管理'),
    '财务协同/盈亏报表.html': ('财务看板', '财务管理'),
...
# ---- 目标态菜单标签序列（结构自检基准，v3.5：G07 删两台账 · G09 退租入库入租赁标签 · G10 财务看板置首改名） ----
EXPECT_SEQ = ['项目管理', '项目看板', '项目列表', '我的待办', '基础资料', '客商管理', '产品档案', 'BOM', '库位档案',
              '采购管理', '采购订单', '采购入库', '销售管理', '销售订单', '销售出库',
              '租赁管理', '租赁', '租赁单', '租赁出库', '退租入库', '租入', '租入单', '租入入库', '租入归还',
              '仓储管理', '库存管理', '库存查询', '盘点', '库存调拨', '入库类', '其他入库', '出库类', '其他出库',
              '财务管理', '财务看板', '应收', '应收账单', '开票登记', '收款登记', '收款核销', '应付', '应付账单', '付款登记',
              '系统管理', '用户权限', '角色管理', '操作日志', '数据字典']
LABELS = [x for x in EXPECT_SEQ if x not in ('租赁', '租入', '库存管理', '入库类', '出库类', '应收', '应付')]
GROUPS = ['项目管理', '我的待办', '基础资料', '采购管理', '销售管理', '租赁管理', '仓储管理', '财务管理', '系统管理']
...
for rel in changed:
    ...
    for old in ['财务应收', '财务应付', '>组合出库</div>', '>盘点录入</div>', '包装管理', '器具档案', '零部件档案']:
        assert old not in aside, f'{rel}: 旧菜单残留 {old}'
    # v3.2 旧组名形态不得残留（目录路径内的“财务协同/仓储作业”属正常，不在此形态）
    assert '</span>财务协同<span' not in aside, f'{rel}: 旧组名形态残留 财务协同'
    assert '</span>仓储作业<span' not in aside, f'{rel}: 旧组名形态残留 仓储作业'
