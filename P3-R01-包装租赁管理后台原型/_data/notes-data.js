...
window.NOTES_DATA = {
  ...
  "项目管理/项目档案.html": [
    ...
    },
    {
      ...
      "title": "上下游绑定（弹窗）",
      "note": "项目↔客户↔供应商关系维护（一对多/多对多，2026-09-11 落地：单项目可绑定多家供应商，勾选含租入/采购角色）；单据按此带出候选供应商",
      "fp": "FP2-02",
      "req": "REQ-01"
    },
    {
      ...
      "title": "上下游绑定关系口径",
      "note": "上下游支持一对多/多对多（2026-09-08 王总提出，2026-09-11 落地）：单项目可绑定多家供应商（勾选，含租入/采购角色），新建租入单/采购订单时按项目带出候选供应商。",
      "fp": "FP2-02",
      "req": "REQ-01"
    }
  ],
  ...
  "基础数据/库位档案.html": [
    {
      ...
      "title": "库位档案",
      "note": "库位两级＝库位（仓区）＋库位编码（D-102 去库区层级·2026-09-14 第4次沟通）；2026-09-17 收口：库位列取库区命名（原料区 RA/成品区 RB/次品区 RC＋直发虚拟仓 XNC-ZF（D-167）），实际损耗走次品区 RC 锁定不参与拣货（D-101）；库区层级后续做大了再加",
      "fp": "FP1-06",
      "req": "REQ-04"
    },
    {
  ...
  "采购管理/采购入库列表.html": [
    {
      ...
      "title": "采购入库单",
      "note": "按物料基本单位入库（D-145）；入库即入零件级库存",
      "fp": "FP3-01",
      "req": "REQ-04"
    },
    {
  ...
  "租赁管理/租赁出库列表.html": [
    ...
    },
    {
      ...
      "title": "关联销售订单",
      "note": "出库与客户订单关联，退回进度自动记入租赁单列表（原租出台账·2026-09-10 并入）并生成应收",
      "fp": "FP3-03",
      "req": "REQ-03"
    }
  ],
  ...
  "仓储作业/库存查询.html": [
    ...
    },
    {
      ...
      "title": "退租在途（无独立审核）",
      "note": "客户已通知退租 500 件 · 退租入库待确认 7 单。\n退租申请模块已移除（2026-09-08 会议）：退租只做两件事——在途（客户通知）→ 退租入库（改库存），无独立审核环节。",
      "fp": "FP3-06",
      "req": "REQ-06"
    },
    {
  ...
  "基础数据/产品档案.html": [
    {
      ...
      "title": "税率口径与档案合并",
      "note": "口径：同一物料可按供应商维护不同税率（默认 13%；运费/杂费后续可能 6%/9%）；单据明细税率默认带出、可手动覆盖。物料档案 = 器具档案 + 零部件档案合并（2026-09-08 会议 N4）。\n税率在新建物料弹窗内按供应商维护（行编辑器）；本页不再单列维护表。",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "系统管理/角色管理.html": [
    {
      ...
      "title": "可审单据与审核人匹配",
      "note": "勾选该角色可审核的单据类型；各审核弹窗「审核人（按角色匹配）」据此匹配（G13 · 纪要十.1）。",
      "fp": "FP7-01",
      "req": "REQ-01"
    }
  ],
  ...
  "mobile/审批详情.html": [
    ...
    },
    {
      ...
      "title": "审批动作口径",
      "note": "通过后单据进入下一环节并通知提交人；驳回后退回提交人修改后重新提交。移动端为演示形态。"
    }
  ],
  "基础数据/客商管理.html": [
    {
      ...
      "title": "新建客商",
      "note": "客户/供应商两类主体（运营方角色已去掉，2026-09-08 会议 L6）；开票资料独立弹窗维护（含结算周期）",
      "fp": "FP1-02",
      "req": "REQ-01"
    }
  ],
  "租入管理/租入入库列表.html": [
    {
      ...
      "title": "直发自动生成租赁出库",
      "note": "D-167 类型驱动（09-18 袁工口径，原 D-105 入库勾选触发口径废止）：租入单类型=直发时，入库确认后系统自动生成租赁出库单（自动带物料·免重复填单·租赁出库列表可查 CK-20260914-023）；计费口径不受影响（D-122）",
      "fp": "FP2-01",
      "req": "REQ-02",
      "id": 1
    }
  ...
  "租入管理/租入单列表.html": [
    {
      ...
      "title": "租入单类型",
      "note": "类型=直发（供应商直发客户）/自发（先入我方库再发）；直发审核通过后系统自动生成租入入库与租赁出库（09-18 袁工口径·命名 09-19 复核定稿=直发/自发）"
    }
  ],
  "租入管理/租入入库详情.html": [
    {
      ...
      "title": "直发链自动化",
      "note": "直发链自动化：直发租入单→自动生成本单（入直发虚拟仓·不占实体库存）→自动生成租赁出库并关联（第4次『立即转租』口径 09-18 升级为类型驱动）"
    }
  ],
  "租赁管理/退租入库详情.html": [
    {
  ...
  "租赁管理/转移出库列表.html": [
    {
      ...
      "title": "转移出库单（D-146）",
      "note": "直接客户→终端客户的器具转移，一步式·不勾稽原租赁单（D-106）。审核通过后转移生效，库存状态转「客户转租出」（09-16 拍板加审·审核页承载）；「终止转移」后回「在客户（租出）」。转租登记/转租还回弹窗已退场，由本单承载。结算方式默认取项目档案「转租结算方式」可按单覆盖（D-132）。",
      "fp": "FP3-06",
      "req": "REQ-06"
    }
  ],
  "租赁管理/转移出库新建.html": [
    {
      ...
      "title": "结算方式两值（D-132）",
      "note": "默认取项目档案「转租结算方式」，可单据级覆盖。按租出结算＝租金仍向直接客户计收，转移单不进财务链路；按终端结算＝生效后后续账单主体切终端客户，历史账单不回改。终端客户可选客商档案或手工录入（D-108·不强制建档）。",
      "fp": "FP4-02",
      "req": "REQ-03"
    }
  ],
