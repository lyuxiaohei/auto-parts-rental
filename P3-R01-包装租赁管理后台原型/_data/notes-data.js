...
window.NOTES_DATA = {
  ...
  "基础数据/BOM维护.html": [
    ...
    },
    {
      ...
      "aud": "dev",
      "title": "自动带出字段不加后缀",
      "note": "客户/供应商/所属项目/收货地点等由上游单据或主数据自动带入的字段，字段名与列表同名、不写「（带出）」后缀（0924 道远拍板·原后缀已全站清除）；自动带出的实现由开发保证。"
    }
  ],
  "基础数据/库位档案.html": [
    {
    ...
    },
    {
      "id": 4,
      "aud": "dev",
      "selector": "<span class=\"ff-label\">库位类型：</span>",
      "title": "类型筛选待接线",
      "note": "类型筛选下拉当前为静态装饰未接过滤逻辑，属待追认项；接线前勿据其做数据断言",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "采购管理/采购入库录单.html": [
    ...
    },
    {
      "id": 3,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">基本信息</h3>",
      "title": "批量设置库位件",
      "note": "按 th 文本找列注入 select；克隆到含下拉列的页面必须补 select 分支，否则整列控件被毁",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "租赁管理/租赁出库录单.html": [
    ...
    },
    {
      "id": 3,
      "aud": "dev",
      "selector": "<th>可用组合库存</th>",
      "title": "库存校验统一件",
      "note": "明细数量输入须挂 data-tax=qty；可用库存列由 .stock-cell 紧挂数量列渲染，量源 STOCK_MAP 单源；新页克隆勿漏",
      "fp": "",
      "req": ""
    },
    {
  ...
  "租赁管理/租赁单列表.html": [
    {
      "id": 6,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">租赁单</h3>",
      "title": "库存校验统一件",
      "note": "明细数量输入须挂 data-tax=qty；可用库存列由 .stock-cell 紧挂数量列渲染，量源 STOCK_MAP 单源；新页克隆勿漏",
      "fp": "",
      "req": ""
  ...
  "仓储作业/库存查询.html": [
    ...
    },
    {
      ...
      "title": "客户虚拟仓与在租明细",
      "note": "客户虚拟仓＝在客户处的租赁资产按客户归集（客户端（租出）口径）；客户转租为其子状态。\n客户在租明细＝经物料行「详情」进入物料详情页「客户在租（本物料）」区块（原弹窗 0923 拍板并入）；租出与退回进度在租赁单列表「退回进度」列跟踪。\n对应在租租赁单 40 张。",
      "fp": "FP4-02",
      "req": "REQ-03"
    },
    {
      ...
      "selector": "<div class=\"st-label\"><span>在客户</span></div>",
      "title": "客户虚拟仓口径沿革",
      "note": "客户转租为子状态（2026-09-08 会议 T1 方向）；客户在租明细原在租台账（2026-09-10 并入）；0923 拍板「库位明细/客户在租」弹窗并入物料详情页（G61）。"
    },
    {
      "id": 4,
  ...
  "财务协同/应收账单.html": [
    ...
    },
    {
      "id": 3,
      "aud": "dev",
      "selector": "<td><span class=\"lk\">BS-20260828-004</span></td>",
      "title": "详情三卡化待拍板",
      "note": "本模块详情走专用渲染器（期段化账单），尚未切 formRows 三卡版式；切换前勿按三卡口径改造本模块数据",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "系统管理/数据字典.html": [
    ...
    {
      "id": 2,
      "selector": "<div class=\"dic-item\" data-cat=\"计费方式\">",
      "title": "计费方式",
  ...
  "基础数据/产品档案.html": [
    ...
    },
    {
      "id": 3,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">物料档案</h3>",
      "title": "列显示设置件",
      "note": "列显隐由 CSS nth-child 实现，勾选状态存 localStorage（键 ksgl/wlda）；恢复默认须带可见反馈；克隆到新页须换存储键并逐列对齐 th 文本",
      "fp": "",
      "req": ""
    },
    {
      "id": 4,
      "aud": "dev",
      "selector": "<th colspan=\"2\">参考未税采购价 / 销售价(元)</th>",
      "title": "参考价与单据价脱钩",
      "note": "四参考价仅档案参考，与全部录单点单价零联动（单价手工录入）；联动规则与 6 条待定事项悬置未拍板，勿自行实现联动",
      "fp": "",
      "req": ""
    }
  ],
  "仓储作业/收发存.html": [
    {
      "id": 1,
      "selector": "<h3 class=\"card-title\">库存台账</h3>",
      "title": "台账口径与分工",
      "note": "期初结转＝历史在租结转；退租入库＝退租回库事件；租赁出库＝租赁/销售出库事件；归还出库＝租入归还供应商；期末结存＝在库＋退租在途＋客户端(租出)＋退租待入库。转移出库不单列（仅虚拟仓数据记录、不动实物）；双向不定的其他出入库不计列。\n本页＝期间口径（收/发/结存），对账用；实时库存与在租分布请用「库存查询」；仓库/库区筛选按库位行过滤（收发按物料计）。",
      "fp": "",
      "req": ""
    },
    {
      ...
      "selector": "<th>期末结存</th>",
      "title": "轨迹收敛与排期",
      "note": "G61（0923 拍板）：行内「轨迹」弹窗并入物料详情页——行内「详情」直达（原弹窗已删）。\n退租关联闭环后本页数据完整，开发排期待定（0923 拍板：先画进原型）。",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "仓储作业/其他出库新建.html": [
    {
      "id": 1,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">出库明细</h3>",
      "title": "批量设置库位件",
      "note": "按 th 文本找列注入 select；克隆到含下拉列的页面必须补 select 分支，否则整列控件被毁",
      "fp": "",
      "req": ""
  ...
  "仓储作业/其他入库新建.html": [
    {
      "id": 1,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">入库明细</h3>",
      "title": "批量设置库位件",
      "note": "按 th 文本找列注入 select；克隆到含下拉列的页面必须补 select 分支，否则整列控件被毁",
      "fp": "",
      "req": ""
  ...
  "仓储作业/其他入库列表.html": [
    {
      "id": 1,
      "selector": "<h3 class=\"card-title\">其他入库单</h3>",
      "title": "报数调整单",
      "note": "客户报数调整类入库单用于对账面数量的客户报数校准，走人工录入，不与库存盘点勾稽",
      "fp": "",
      "req": ""
    }
  ],
  "财务协同/应付账单.html": [
    ...
    },
    {
      "id": 2,
      "aud": "dev",
      "selector": "<td>AP-20260903-010</td>",
      "title": "详情三卡化待拍板",
      "note": "本模块详情走专用渲染器（期段化账单），尚未切 formRows 三卡版式；切换前勿按三卡口径改造本模块数据",
      "fp": "",
      "req": ""
  ...
  "财务协同/付款登记.html": [
    {
      "id": 2,
      "selector": "<h3 class=\"card-title\">付款登记</h3>",
      "title": "付款与应付口径",
      "note": "应付账单由采购入库/租入周期生成；付款登记核销应付，多付部分转预付留抵，不自动退款",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "租赁管理/租赁单新建.html": [
    ...
    {
      "selector": "<th style=\"font-size:13.5px;font-weight:700;\">含税单价(元)</th>",
      "title": "免费借用形态",
    ...
    },
    {
      ...
      "selector": "<th>计费方式</th>",
      "title": "计费方式直录",
      "note": "按时间周期计费的直录日租金，按次计费的录次单价；三段式租价仅作物料档案参考价，单据侧不做换算",
      "fp": "",
      "req": ""
    },
    {
  ...
  "基础数据/客商管理.html": [
    ...
    },
    {
      "id": 2,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">客商管理</h3>",
      "title": "列显示设置件",
      "note": "列显隐由 CSS nth-child 实现，勾选状态存 localStorage（键 ksgl/wlda）；恢复默认须带可见反馈；克隆到新页须换存储键并逐列对齐 th 文本",
      "fp": "",
      "req": ""
  ...
  "基础数据/客商新建.html": [
    {
      "id": 2,
      "selector": "<div class=\"form-label\"><span class=\"req\">*</span>结算周期：</div>",
      "title": "结算周期与发票类型值源",
      "note": "结算周期取数据字典统一值源（结算周期组·与物料档案供应商税率区共用）；发票类型＝「增值税专票 × 供应商税率字典各档」＋增值税普通发票。",
      "fp": "",
      "req": ""
    },
    {
      ...
      "aud": "dev",
      "title": "协议税点退场与结算周期归位",
      "note": "协议税点（原 9%/13% 多选 chips）已删除（0924 道远拍板）；发票类型税率来源改「供应商税率」字典各档；结算周期为客商级字段，已移至基础信息卡（原在开票资料卡、组外）。"
    }
  ],
  "基础数据/客商详情.html": [
    {
      "id": 1,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\" id=\"dtTitle\">客商详情</h3>",
      "title": "详情数据驱动",
      "note": "本页内容全部由 demo-data partners 按 formRows 数据驱动（信息区 sec 拆卡＝客商信息/开票资料/收货信息三卡＋明细卡＋流转卡）；调字段改数据键，不动页面 HTML",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "租入管理/归还出库列表.html": [
    {
      "id": 3,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">归还出库单</h3>",
      "title": "更名沿革",
      "note": "原『租入归还』，09-18 会议后更名『归还出库』（与租入入库成对）；文件名/URL/单据前缀 GHCK 不变，仅展示名变更",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "租赁管理/退租入库新建.html": [
    {
      "id": 1,
      "selector": "<h3 class=\"card-title\">退租入库信息</h3>",
      "title": "计费天数规则",
      "note": "计费天数＝止租日−起租日＋1，且不足 2 天按 2 天计（当天起租当天退回＝2 天）；止租日当天仍按持有量计租",
      "fp": "",
      "req": ""
    },
    {
      "id": 2,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">退回明细</h3>",
      "title": "批量设置库位件",
      "note": "按 th 文本找列注入 select；克隆到含下拉列的页面必须补 select 分支，否则整列控件被毁",
      "fp": "",
      "req": ""
  ...
  "采购管理/采购退货单列表.html": [
    ...
    },
    {
      ...
      "title": "新建退货单",
      "note": "选择关联采购入库单号自动带出物料行与可退上限；退货类型两值（THC 字典）。",
      "fp": "",
      "req": ""
    },
    {
      ...
      "selector": "<h3 class=\"card-title\">采购退货单</h3>",
      "title": "退货退款守卫",
      "note": "已付款的退货可发起退款，退款累计不超已付额；未付款的退货不立退款单，冲减应付按实际入库量重算",
      "fp": "",
      "req": ""
    }
  ],
  "采购管理/采购订单列表.html": [
    {
      "id": 2,
      "selector": "<h3 class=\"card-title\">采购订单</h3>",
      "title": "交期口径",
      "note": "列表日期列为要求到货日期（交期），非下单日期；供应商提前到货属正常业务，不作异常处理",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "销售管理/销售出库列表.html": [
    {
      "id": 2,
      "selector": "<h3 class=\"card-title\">销售出库单</h3>",
      "title": "出库关联应收",
      "note": "已完成的销售出库自动挂接应收账单，一张出库对应一条应收明细，可从行内追溯；待审核出库不挂应收",
      "fp": "",
      "req": ""
    }
  ],
  ...
  "销售管理/销售订单列表.html": [
    {
      "id": 2,
      "aud": "dev",
      "selector": "<h3 class=\"card-title\">销售订单</h3>",
      "title": "库存校验统一件",
      "note": "明细数量输入须挂 data-tax=qty；可用库存列由 .stock-cell 紧挂数量列渲染，量源 STOCK_MAP 单源；新页克隆勿漏",
      "fp": "",
      "req": ""
