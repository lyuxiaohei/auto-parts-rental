...
> **版本**：V1.1（2026-09-11 · G16a 拆图版：第 5 节单图拆为 1 底图+4 流程线·陆鸣拍板方案 A·新增同名 HTML 渲染版 7 图）  
...
| 租赁·租出 | `returnInbounds` | 退租入库 | 租赁管理/退租入库列表.html（**形态变更 0923·D-176**：取消自由新建→两步选单〔先选客户→选原租赁出库单带出明细·退租数量≤原出库〕·关联原出库单·反转 D-106/D-152④；**清洗审核动作 D-177**：清洗完工→待审核→审核通过→转可用库存·列表批量审核〔审核节点非库存状态〕） | 10 |
| 租赁·租出 | `transferOutbounds` | 转移出库单 | 租赁管理/转移出库列表.html（G37·D-146·状态流 待审核→已转移→已终止（D-151 加审）；**0923·D-178**：物料粒度不挂单号·虚拟化仅数据记录·详情「财务口径」=按客户结算口径生成——按租出客户在租赁出库侧/按转移客户在转移出库侧·单一模块生成不拆双账〔口径挂客商档案 partners〕） | 5 |
| 租赁·租出 | `rentTracks` | 出租履历 | 库存查询·出租履历弹窗 | 10 |
| 租赁·租入 | `rentInOrders` | 租入单 | 租入管理/租入单列表.html | 6 |
| 租赁·租入 | `rentInbounds` | 租入入库 | 租入管理/租入入库列表.html | 3 |
| 租赁·租入 | `rentInReturns` | 归还出库 | 租入管理/归还出库列表.html | 3 |
...
| 财务 | `receipts` | 收款登记 | 财务协同/收款登记.html | 5 |
...
| 财务 | `profitRows` | 项目损益 | 财务协同/损益报表.html | 6 |
...
| 2 | partners → products（供应商税率维护区） | 1:N | 数据=products 6/12 行带 supplier 名（吴越联合/甬城塑业/延陵托盘）；名字外键（缺口 #3） |
| 3 | partners → purchaseOrders | 1:N | 数据=partners 3 条记录被 PO- 引用；purchaseOrders.fields.supplier=供应商名 |
| 4 | partners → rentInOrders / payableBills | 1:N | 数据=payableBills.fields.supplier（11 行）；rentInOrders 供应商=环通（A04 流程链 L3） |
...
| 16 | rentInOrders → rentInReturns（归还出库） | 1:N | 数据=rentInReturns 3/3 行含 RZD-（关联租入单+明细同步+分批） |
...
| 25 | receivableBills / receipts → writeoffs（银行回单核销） | N:M | 数据=writeoffs 3 行含 AR-；核销=收付款与账单多对多冲抵（银行回单核销页） |
| 26 | stocktakes → otherInbounds / otherOutbounds（盘损益生成其他出入库） | 1:N | 数据=otherInbounds 1 行含 PD-、otherOutbounds 1 行含 PD-（2026-09-11 盘点联动；关联单据列） |
| 27 | locations → stockFlows（虚拟仓） | 1:N | 数据=stockFlows XNC-* 4 行键=历史客户虚拟仓编码 XNC-AJZX（D-006 前口径·历史流水保留）；**G54 起含系统内置直发虚拟仓 XNC-ZF**（D-167·直发租入入库/退租归还账面枢纽·不占实体库存·区别于 XNC-ZZ 转移物料编码） |
...
| `receipts` 收款登记 | 待核销 / 部分核销 / 已核销 |
| `invoices` 开票登记 | 已红冲 / 已登记 / 停用 |
| `writeoffs` 银行回单核销 | 部分核销 / 已核销 |
...
2. **stockFlows 无供应商维度**——租入在库无法按供应商查；环通对账需补 supplier 字段。
...
## 5. 实体关系图（拆分版 · 2026-09-11 陆鸣拍板方案 A：1 底图 + 4 流程线）
...
flowchart LR
  ...
  LOCATIONS -->|"直发虚拟仓 XNC-ZF（D-167）·历史 XNC- 4 行"| STOCK
  STK -->|"盘盈生成（PD-）"| OIN
  STK -->|"盘亏生成（PD-）"| OOUT
...
flowchart LR
  ...
  GH["rentInReturns·归还出库 (3)"]
  AP["payableBills·应付账单 (11)"]
  PM["payments·付款登记 (5)"]
  ZL0["leaseOrders·租赁单〔复现〕"]
  SG["partners·供应商（环通）〔复现〕"]
  RZD -->|"租入入库（3/3 RZD-）"| ZR
  RZD -->|"归还出库（3/3 RZD-·明细同步分批）"| GH
  RZD -->|"租金应付（2 行 RZD-）"| AP
  GH -->|"赔付应付"| AP
...
> 跨线关系速查：销售⇄采购（联动采购 2 行 SO-）｜租赁⇄租入（客户转租 XNC-ZZ 3 行）｜盘点⇄其他出入库（损益生成）——均为虚线口径，证据详见第 2 节关系表 #12/#17/#24。
